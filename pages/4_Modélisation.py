...
def main():
    ...
    "Region","MomentOfTheDay","PropertyType","AttendanceTime"]]
        ...
        st.markdown("""
            ...
            Cependant, parfois, certains ingrédients peuvent prendre le dessus et gâcher le plat. Pour éviter cela, la régression Ridge "<span style='color:yellow'>pénalise</span>" les ingrédients trop dominants, c'est-à-dire qu'elle réduit leur importance dans la recette. 
            Le but est d'obtenir un plat (une prédiction) qui est un bon équilibre de tous les ingrédients (données d'entrée), plutôt que d'être dominé par un ou deux ingrédients. """, unsafe_allow_html=True)
